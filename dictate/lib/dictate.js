(function(window){
    // Defaults
    var SERVER = "ws://140.122.x.x:5678";
    var SERVER_STATUS = "ws://140.122.x.x:5678";
    // TODO: how to send a transcript?
    var REFERENCE_HANDLER = "http://140.122.x.x:5678";
    ...
    var Dictate = function(cfg) {
        ...
        this.startListening = function() {
            ...
            try {
                ...
                audioContext.resume().then(() => {
                    config.onEvent(MSG_AUDIOCONTEXT_RESUMED,
        ...
        function createWebSocket() {
            ...
            //ws.send("WebSocket: onEvent: got Object that is not a Blob");
                    ...
                    if (res.status == 0) {
                        if (res.result) {
                            if (res.result.final) {
                                ...
                                var asr_results_str = res.result.hypotheses.transcript;
                                var asr_result_list = asr_results_str.split(" ");
                                config.onResults(asr_result_list);
                                
                            } else {
                                config.onPartialResults(res.result.hypotheses.transcript);
                            }
                        }
                    } else {
