...
class Application(tornado.web.Application):
    ...
    def send_status_update_single(self, ws):
        ...
        try:
            ...
            logging.warn("Exception occured when sending status update")
            pass
...
class HttpChunkedRecognizeHandler(tornado.web.RequestHandler):
    ...
    def prepare(self):
        ...
        self.prons_length = self.request.headers.get("self.prons_length", "none")
        logging.info("%s: OPEN: user='%s', content='%s'" % (self.id, self.user_id, self.content_id, self.prons_length))
        self.worker = None
        self.error_status = 0
        ...
        try:
            ...
            self.worker.write_message(json.dumps(dict(id=self.id, content_type=content_type, user_id=self.user_id, content_id=self.content_id, prons_length=self.prons_length)))
        except KeyError:
...
class DecoderSocketHandler(tornado.websocket.WebSocketHandler):
    ...
    def send_event(self, event):
        ...
        if len(event_str) > 100:
            event_str = event_str[:97] + "..."
        logging.info("%s: Sending event back to client" % (self.id))
        logging.info("%s: >> %s" % (self.id, event_str))
    ...
    def open(self):
        ...
        self.prompt = self.get_argument("prompt", "none", True)
        self.prons_length = self.get_argument("prons_length", "none", True)
        logging.info("%s: prompt : %s" % (self.id, self.prompt))
        self.worker = None
        try:
            self.worker = self.application.available_workers.pop()
            self.application.send_status_update()
            ...
            self.worker.write_message(json.dumps(dict(id=self.id, content_type=content_type, 
                                      prompt=self.prompt, user_id=self.user_id, content_id=self.content_id, prons_length=self.prons_length)))
        except KeyError:
            logging.warn("%s: No worker available for client request" % self.id)
...
def main():
    ...
    define("certfile", default=os.path.join(os.getcwd(), "fullchain.pem"), help="certificate file for secured SSL connection")
    define("keyfile", default=os.path.join(os.getcwd(), "privkey.pem"), help="key file for secured SSL connection")
